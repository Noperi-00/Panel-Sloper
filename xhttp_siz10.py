...
from main import (
    detect_device as _device_from_ua,
    LINKS,
    LINKS_LOCK,
    connections,
    error_logs,
...
class _QuotaGate:
    ...
    """
    __slots__ = ("batch_bytes", "last_check", "ok", "pending", "rate_ewma", "uuid")
...
def _req_ua(request: Request) -> str:
    return request.headers.get("user-agent") or request.headers.get("User-Agent") or ""
...
async def _get_or_create_session(uuid: str, mode: str, session_id: str, ip: str = "نامشخص", ua: str = "") -> dict:
    ...
    async with XHTTP_LOCK:
        ...
        if sess is not None:
            if sess.get("uuid") != uuid or sess.get("mode") != mode or sess.get("ip") != ip:
        ...
        sess = {
            ...
            "last_seen": time.time(),
            "conn_id": conn_id, "ip": ip, "tcp_open": False, "closed": False,
            "seq_buf": {}, "next_seq": 0,
...
async def xhttp_downlink(mode: str, uuid: str, session_id: str, request: Request):
    ...
    sess = await _get_or_create_session(uuid, mode, session_id, _req_client_ip(request), _req_ua(request))
    if sess.get("closed"):
...
async def packet_up_upload(uuid: str, session_id: str, seq: int, request: Request):
    ensure_reaper()
    sess = await _get_or_create_session(uuid, "packet-up", session_id, _req_client_ip(request), _req_ua(request))
    if sess.get("closed"):
        raise HTTPException(status_code=404, detail="session closed")
...
async def stream_up_upload(uuid: str, session_id: str, request: Request):
    ensure_reaper()
    sess = await _get_or_create_session(uuid, "stream-up", session_id, _req_client_ip(request), _req_ua(request))
    if sess.get("closed"):
        raise HTTPException(status_code=404, detail="session closed")
